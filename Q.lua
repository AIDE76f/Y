--[[
    Muscle Legends - Auto Farm GUI (القائمة الأولى)
    المكان: Muscle Legends (Place ID: 3623096087)
    الميزات: Auto Train, Auto Collect Coins, Auto Rebirth, Teleport, Anti-AFK
--]]
...
if game.PlaceId ~= 3623096087 then
    warn("هذا السكريبت مخصص للعبة Muscle Legends فقط!")
    return
end
...
local Settings = {
    AutoTrain = {
        Enabled = false,
        TrainSpeed = 50,  -- 1-100 (سرعة التدريب)
        TrainStrength = true,
        TrainAgility = true,
        TrainStamina = true
    },
    AutoCollect = {
        Enabled = false,
        CollectRange = 50  -- نطاق الجمع بالستوديوت
    },
    AutoRebirth = {
        Enabled = false,
        RebirthAt = 10000  -- القوة المطلوبة لإعادة الميلاد
    },
    Teleport = {
        Enabled = false,
        SelectedGym = "Tiny Island"  -- الصالة الافتراضية
    },
    AntiAFK = {
        Enabled = true  -- منع الطرد بسبب عدم النشاط
    }
}

-- قائمة الصالات الرياضية المتاحة للانتقال السريع
local Gyms = {
    "Tiny Island",
    "Frost Gym",
    "Mythical Gym",
...
ContentFrame.CanvasSize = UDim2.new(0, 0, 0, 600)  -- حجم المحتوى
...
-- دالة إنشاء زر تبديل (Toggle) مع مؤشر لوني
...
yPos = CreateToggle(ContentFrame, "تفعيل التدريب التلقائي", 
    ...
    function(v) 
        Settings.AutoTrain.Enabled = v
        if v then
            startAutoTrain()
        else
            if TrainLoop then TrainLoop:Disconnect() end
        end
    end, yPos, "يقوم بتدريب القوة والسرعة والتحمل تلقائياً")
...
yPos = CreateToggle(ContentFrame, "تفعيل جمع العملات", 
    ...
    end, yPos, "يجمع العملات والجواهر من الأرض تلقائياً")
...
yPos = CreateToggle(ContentFrame, "تفعيل إعادة الميلاد", 
    ...
    end, yPos, "يعيد الميلاد تلقائياً عند الوصول للقوة المطلوبة")
...
function startAutoTrain()
    ...
    TrainLoop = RunService.Heartbeat:Connect(function()
        ...
        -- محاكاة النقر للتدريب باستخدام VirtualUser
        pcall(function()
            VirtualUser:Button1Down(Vector2.new(0, 0))
            wait(0.1 / (Settings.AutoTrain.TrainSpeed / 10))  -- السرعة تعتمد على الإعداد
    ...
    CollectLoop = RunService.Heartbeat:Connect(function()
        ...
        for _, part in ipairs(coins) do
            if part:GetAttribute("IsCoin") or part.Name:lower():find("coin") or part.Name:lower():find("gem") then
                -- محاولة جمع العملة
                firetouchinterest(rootPart, part, 0)  -- محاكاة اللمس
                task.wait(0.05)
...
function startRebirthCheck()
    if RebirthCheckLoop then RebirthCheckLoop:Disconnect() end
    
    RebirthCheckLoop = RunService.Heartbeat:Connect(function()
        if not Settings.AutoRebirth.Enabled then return end
        
        -- البحث عن قوة اللاعب في leaderstats
        ...
        if leaderstats then
            ...
            if strength and strength.Value >= Settings.AutoRebirth.RebirthAt then
                ...
                local rebirthRemote = game.ReplicatedStorage:FindFirstChild("Rebirth") 
                    or game.ReplicatedStorage:FindFirstChild("RemoteEvent"):FindFirstChild("Rebirth")
...
function teleportToGym(gymName)
    if not LocalPlayer.Character then return end
    
    local rootPart = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not rootPart then return end
    
    -- البحث عن موقع الصالة
    local gymLocation = workspace:FindFirstChild(gymName) 
        or workspace:FindFirstChild("Gyms"):FindFirstChild(gymName)
    ...
    if gymLocation and gymLocation:FindFirstChild("SpawnLocation") then
        ...
        notify("لم يتم العثور على الصالة", 2)
    end
end
...
AntiAFKLoop = RunService.Heartbeat:Connect(function()
    if Settings.AntiAFK.Enabled then
        -- تحريك بسيط لمنع الطرد
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
            LocalPlayer.Character.Humanoid:Move(Vector3.new(0, 0, 0), true)  -- حركة وهمية
        end
